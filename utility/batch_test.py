...
def get_auc(item_score, user_pos_test):
    ...
    for i in item_sort:
        if i in user_pos_test:
            ...
            r.append(0)
    auc = utility.metrics.auc(ground_truth=r, prediction=posterior)
    return auc
...
def get_performance(user_pos_test, r, auc, Ks):
    ...
    for K in Ks:
        ...
        recall.append(utility.metrics.recall_at_k(r, K, len(user_pos_test)))
        ndcg.append(utility.metrics.ndcg_at_k(r, K, user_pos_test))
        hit_ratio.append(utility.metrics.hit_at_k(r, K))
...
def test_model(model,data_generator):
    ...
    for test_idx in range(data_generator.n_users):
        ...
        rate_batch = model.rating(u_g_embeddings, pos_i_g_embeddings).detach().cpu()[0]*1.0
    ...
    return mean(hr_list),mean((hr_list_20)),mean(ndcg_list),mean(ndcg_list_20)

def client_model_test(model,data_generator,client_model):
    hr_list = []
    ndcg_list = []
    hr_list_20 = []
    ndcg_list_20 = []
    for test_idx,idx_model in client_model.items():

        model.load_state_dict(idx_model)
